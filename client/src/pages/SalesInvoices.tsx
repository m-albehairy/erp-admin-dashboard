...
export default function SalesInvoices() {
  ...
  const [editFormData, setEditFormData] = useState({ invoiceNo: "", customer: "", amount: "", status: "" });

  const totalPages = Math.ceil(salesInvoicesData.length / pageSize);
  const startIndex = (currentPage - 1) * pageSize;
  const paginatedData = salesInvoicesData.slice(startIndex, startIndex + pageSize);
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...salesInvoicesData.map(item => [item.invoiceNo, item.customer, item.amount, item.status, item.date, item.dueDate])
    ...
    a.href = url;
    a.download = "sales-invoices-export.csv";
    a.click();
  };
